...
fn test_integration_project(project_name: &str) {
    ...
    assert!(result.is_ok(), "Compilation failed for project: {}", project_name);
    ...
    assert!(main_rs.exists(), "main.rs not generated for project: {}", project_name);
    assert!(cargo_toml.exists(), "Cargo.toml not generated for project: {}", project_name);
    ...
    assert!(!rust_code.is_empty(), "Generated Rust code is empty for project: {}", project_name);
    
    let cargo_toml_content = std::fs::read_to_string(&cargo_toml).unwrap();
    assert!(!cargo_toml_content.is_empty(), "Generated Cargo.toml is empty for project: {}", project_name);
    
    // Verificar que Cargo.toml tiene la estructura básica
    assert!(cargo_toml_content.contains("[package]"), "Cargo.toml missing [package] section for project: {}", project_name);
    assert!(cargo_toml_content.contains("name ="), "Cargo.toml missing name for project: {}", project_name);
}
