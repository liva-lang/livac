...
> **Próximo objetivo:** **v2.0 release al 100%** — Fase 10 Tier 1 ✅ + Tier 2 10.4 ✅ cerrados. Bench (commit `22a37a5`): Line 1.07x · CSV 1.00x · Word 1.23x · Map 1.09x · Sort/Filter+Map <6ms (ruido DCE/timer). **5 bloques pendientes para 100%:** (1) cross-module `&str` (cierra Word counting <1.15x — refactor `main.liva` con pre-pass global de signaturas para que callers cross-module vean borrow info), (2) 10.5 Box<str> Map values (análisis `_localMapEscape` + codegen `HashMap<K, Box<str>>` + `m.get → &str`), (3) cobertura medida (instalar `cargo-llvm-cov`, baseline, cubrir gaps), (4) E2E self-host (script ejecuta los 85 examples con gen-2 y compara stdout vs gen-1/Rust nativo), (5) limpieza BACKLOG (marcar items obsoletos — L478 self-host codegen ya hecho). Estado validado: 518 cargo tests + bootstrap 9/9 + 135 archivos `.liva` (e2e 61, errors 28, syntax 18, stdlib 19, compile 9) + idempotencia gen-2≡gen-3 (binario+src). Plan completo en `compiler/docs/PLAN.md` § Fase 10 y `BACKLOG.md`.  
