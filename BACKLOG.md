...
- [x] **`-D warnings` en gen-2 emit** — opt-in via `LIVA_STRICT=1` env var (2026-05-21). El prelude por defecto sigue siendo el `#![allow(...)]` amplio para evitar romper código generado; con `LIVA_STRICT=1 livac build foo.liva` se reduce a solo `unused_imports, unused_parens, dead_code, non_snake_case` (los inevitables por el codegen) y los warnings accionables (`unused_variables`, `unused_mut`, `unused_assignments`, `unreachable_code`, `while_true`) afloran via rustc.
